...
if(validationResponce !==true){
  ...
  Comments.create(comment)
    .then((result) => {
      res.status(201).json({ messag: " New Comment created", comment: result });
    })
    .catch((err) => {
      res.status(500).json({ messag: "comment not created", err: err });
    });
};
//!---------------------------------------------//
...
exports.delete_comments = (req, res, next) => {
  ...
  Comments.destroy({ where: { id: id, userId: userId } })
    .then((result) => {
      res.status(200).json({
